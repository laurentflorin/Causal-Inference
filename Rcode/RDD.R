...
distance_centered <- full_data %>% 
  mutate(distance_cutoff_center = distance - 0.3)
gap_measure<- lm(income_fe_t1_self_selection ~ distance_cutoff_center + d_self_selection, data= filter(distance_centered,
                                                                                                       distance_cutoff_center <= 0.05,
                                                                                                       distance_cutoff_center >= -0.05))
tidy(gap_measure)
#Intercept: 4347 --> income at distance_cutoff_center = 0 --> distance = 0.3 & d_self_selection = 0
#d_self_selection: 436 --> treatment jumps 436 from one group to another --> causal effect 


# DATA FILTERED:
#-------------------------------------------------------------------------------
### We select only the treated self-selected under the cutoff value and the 
### non-treated above the cut-off value
#-------------------------------------------------------------------------------
# Using this condition for the self-selection to remove the self-selected 
# from values higher than the cutoff:

d_self_selection <- d_star
for (i in 1:n){
  if ((d_star[i] > 0 & data$distance[i] < 0.3)) {
    d_self_selection[i] <- 1
  } else {
    d_self_selection[i] <- 0
  }
...
full_data_rdd <- full_data[1:15000,] %>% #reducing the sample for visualization purposes
  filter(!(distance < 0.3 & d_self_selection == 0)) %>%  # we remove the non self selected below the cutoff value
  mutate(d_self_selection = factor(d_self_selection)) #factoring for the plot
...
ggplot(full_data_rdd , aes(x= distance, y=income_fe_t1_self_selection, color = d_self_selection))+
  ...
  scale_color_manual(values = c("orange", "blue")) +
  guides(color = guide_legend(override.aes = list(shape = c(15, 15))))+
  geom_smooth(method = "lm", data = filter(full_data_rdd, distance < 0.3 & d_self_selection == 1)) +
  geom_smooth(method = "lm", data = filter(full_data_rdd, d_self_selection ==0))+
  labs(x = "Distance to ZCH", y = "Income", color = "Selection to Treatment")
...
gap_measure<- lm(income_fe_t1_self_selection ~ distance_cutoff_center + d_self_selection, data= filter(distance_centered,
                                                                                                        ...
                                                                                                        distance_cutoff_center >= -0.05))
tidy(gap_measure)
#Intercept: 4350 --> income at distance_cutoff_center = 0 --> distance = 0.3 & d_self_selection = 0
#d_self_selection: 429 --> treatment jumps 429 from one group to another --> causal effect 
